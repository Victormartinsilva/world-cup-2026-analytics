...
_OVERRIDES: dict[str, str] = {
    ...
    "Andreas Pereira":       "Andreas_Pereira_(footballer)",
    ...
    "Emiliano Martínez":     "Emiliano_Martínez_(footballer,_born_1992)",
    ...
    "Dani Olmo":             "Dani_Olmo",
    "Nico Williams":         "Nico_Williams_(footballer,_born_2002)",
...
def _fetch_wiki_photo(name: str) -> str:
    ...
    try:
        r = requests.get(f"{_WIKI_API}/{slug}", headers=_HEADERS, timeout=5)
        if r.status_code == 200:
            src = r.json().get("thumbnail", {}).get("source", "")
            # Upgrade to larger thumbnail (320px wide)
            if src:
                src = src.replace("/50px-", "/320px-").replace("/100px-", "/320px-")
            return src
    except Exception:
        pass
    return ""
...
def load_photos(player_names: list[str]) -> dict[str, str]:
    ...
    Reads from disk cache; only fetches Wikipedia for names not yet cached.
    """
    cache: dict[str, str] = {}
    ...
    missing = [n for n in player_names if n not in cache]
    for name in missing:
